...
def get_yad2_content():
    ...
    for i in range(1, 36):
        item = soup.find("span", {"id": 'feed_item_' + str(i) + '_title'})
        item_id = soup.find(
            "div", {"id": 'feed_item_' + str(i)})['item-id']
        cars_details.append({"details_link": f'https://www.yad2.co.il/item/{str(item_id)}',
                             "item_id": item_id,
                            "item_tiitle": item.span.string.replace('\n', '').strip()
                             })
    return cars_details
...
def getFilteredUrl(user_choices):
    yad2_URL = 'https://www.yad2.co.il/vehicles/cars?'
    user_choices_string = ""
    for category in user_choices.keys():
        if user_choices[category]:
            key, value = category, user_choices[category]
            user_choices_string += key + "=" + ",".join(value) + "&"

    return yad2_URL + user_choices_string


# https://gw.yad2.co.il/search-options/vehicles/cars?fields=manufacturer,year,area,km,ownerID,seats,engineval,engineType,group_color,gearBox
